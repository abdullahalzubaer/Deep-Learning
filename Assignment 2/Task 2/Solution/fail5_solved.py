...
# CHANGE: Removed the softmax activation
layers.append(tf.keras.layers.Dense(
    10,
    kernel_initializer=tf.initializers.RandomUniform(minval=-0.01,
                                                     maxval=0.01)))
...
for step in range(2000):
    ...
    with tf.GradientTape() as tape:
        ...
        for layer in layers:
            out = layer(out)
        # From tf documentation "Warning: This op expects unscaled logits, since it performs a softmax on logits internally for efficiency.
